...
.factory('TwitterLib', function ($rootScope, $q, $window, $http, myAppConfig, $state) {
  ...
  var twitterKey = 'ZXX9glOD3egseoxKRcglSbn2K ';
  ...
  var Twitter = {
    ...
    inAppBrowserLoadHandler: function (_url, _deferred) {
      ...
      var successHandler = function (_args) {
        ...
        var res = _args.text.split("&");
        var oauth_token = res[0];
        var oauth_token_secret = res[1];
        var res1 = oauth_token.split("=");
        
        var res2 = oauth_token_secret.split("=");
        localStorage.setItem("oauth_token", res1[1]);
        localStorage.setItem("oauth_token_secret", res2[1]);
        console.log(res1);
        console.log(res2,"==========");
